const run = () => {
  ...
  (setButtonAnimations = () => {
    for (let button of buttons) {
      button.onclick = function buttonClicker() {
        ...
        setTimeout(function() {setButtonAnimations()}, 1600); // Prevent button spam
      }
    }
  })();
  ...
  const pageTransition = (clickedButton) => {
    ...
    if (menuScreen) {
      ...
      switch (clickedButton.id) {
        ...
        case 'Projects':
          ...
          if (chessProject === null) {
            ...
            setTimeout(function(){chessProject.style.opacity = 1 }, 10);
          }
          ...
          if (projectBanner === null) {
            ...
            setTimeout(function(){projectBanner.style.opacity = 1 }, 10);
          }
          break;
      ...
      clickedButton.setAttribute("style", "margin-left: 20%; left: 0; transform: translate(0,0);\
      font-size: 1.5cm; height: 100px; top: 0; margin-top: 170px;");
  
      arrayButtons[0].setAttribute("style", "margin-left: 0; background: rgba(255, 255, 255, 0.6);width: 300px; height: 120px;\
      margin-top: 0; border: 5px solid black; font-size: 1.5cm; height: 100px; top: 30px; left: 20%; transform: translate(0,0);");
      arrayButtons[1].setAttribute("style", "background: rgba(255, 255, 255, 0.6); width: 300px; height: 120px;  margin-top: 0;\
      border: 5px solid black; font-size: 1.5cm; height: 100px; margin-left: 0; left: calc(20% + 310px); top:30px; transform: translate(0,0);");
    } 
      ...
      for (let project of projects) {
        ...
        setTimeout(function() {project.style.display = 'none'}, 2000)
      }
      for (let button of buttons) {
        button.style = null;
      }
  ...
  const blender = (count = (menuScreen) ? 3 : 37) => {
    if (count < 1 || count > 39) {return;}; // If finished all picturess
    console.log(count);

    if (count % 2 === 0) { // Alternate between two bg layers
      bgF.style.opacity = "100%";
      bg.style.opacity  = "0%";
      bg.style.backgroundImage  = "url(Images/"+buttonColors[clickedButton]+"/"+count+".png)";
    } else {
      bg.style.opacity = "100%";
      bgF.style.opacity = "0%";
      bgF.style.backgroundImage = "url(Images/"+buttonColors[clickedButton]+"/"+count+".png)";
    }
    if (menuScreen && count === 32) { // Reopen the drawer on moving back to menu screen
      openDrawer();
    }
    setTimeout(function () { (menuScreen) ? blender(--count) : blender(++count) } , 50); //Change picture every 50ms
  }
  ...
  const buildChessProject = () => {
    ...
    setTimeout(function(){chessProject.style.opacity = '100%'}, 10); // Required sleep for dynamic element styling
  }
};
